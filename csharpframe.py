...
from assetlib import csharp_pic
class CsharpFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        self.master = master
        self.setup_csharp_frame()

    def setup_csharp_frame(self):
        self.master.change_geometry("400x600")

        self.csharp_label = ctk.CTkLabel(self, text="C#" ,
                                               anchor='center',font=("Old English Text",40,"bold"))
        self.csharp_label.place(relx=0.45, rely=0.015)
        self.csharp_logo_label = ctk.CTkLabel(self, image=csharp_pic, text="",
										                 font=ctk.CTkFont(size=20, weight="bold"))
        self.csharp_logo_label.place(relx = 0.32 , rely = 0.013 )
        
        self.back_main_menu = ctk.CTkLabel(self,text='<--',font=("Old English Text",20,"bold"))
        self.back_main_menu.place(relx=0.2,rely=0.02)
        self.back_main_menu.bind("<Button-1>", lambda event: self.master.open_main_menu_frame("csharp_frame"))
