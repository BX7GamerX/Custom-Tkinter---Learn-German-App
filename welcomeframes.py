...
class WelcomeFrame(ctk.CTkFrame):
    ...
    def setup_welcome_frame(self):
            ...
            self.welcome_label.bind("<Enter>", lambda event: self.welcome_label.configure(cursor="hand2",text_color="green"))
            self.welcome_label.bind("<Leave>", lambda event: self.welcome_label.configure(cursor="arrow",text_color="#92c3c9"))
            ...
            self.show_password = ctk.CTkCheckBox(master=self.welcome_frame, 
                                                 ...
                                                 self.show_password_var), variable=self.show_password_var)
            ...
            self.forgot_label.bind("<Enter>", lambda event: self.forgot_label.configure(cursor="hand2",text_color="green"))#binds the label to the lambda func that changes the cursor
            ...
            self.app_version_label = ctk.CTkLabel(self.welcome_frame, text=app_version,
                                                                font=('Century Gothic', 10))
            self.app_version_label.bind("<Enter>", lambda event: self.app_version_label.configure(cursor="hand2",text_color="blue"))
            self.app_version_label.bind("<Leave>", lambda event: self.app_version_label.configure(cursor="arrow",text_color="white"))
    ...
    def check_passcode(self):
        ...
        # try:
            if self.pass_code_entry.get() != "":
               if confirm_passcode(self.pass_code_entry.get(),passcode):# confirm_passcode(self.pass_code_entry.get(),cfille.readlines()[2]):# or 
