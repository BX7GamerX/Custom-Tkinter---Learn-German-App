...
def Translation_game():
    ...
    ger_array = []
    upper_limit = 0
    word_type = input("chooose word type: ")
    if word_type == 'noun':
        upper_limit = len(english_nouns)
        eng_array = english_nouns
        ger_array =deutsch_nouns
    ...
    score = 0
    count = 0
    while let_him_cook and trials_left >=0:
        golden_index  = random.randint(0,upper_limit-1)
        word_guess = ger_array[golden_index]
       

        trial = input("What is the english translation of '" + word_guess + "' :")

        if trial == eng_array[golden_index]:
            score+=1
            trials_left +=1
            print ('correct keep going!' )
        elif trial == 'show':
            
            print ("pathetic , the answer was :'"+ eng_array[golden_index] + "' Trials left: " + str(trials_left))
        elif trial  == 'quit':
            break
        else :
            trials_left -=1
            print ("wrong the answer is: '" + eng_array[golden_index] + "!' you have :"+ str(trials_left) + " trials left")
            
        
        count +=1
    score_per = ((score/count)*100)

    print ('You scored ' + str (score_per))

#Translation_game() 
word_count = (len(deutsch_adjectives) + len(deutsch_nouns) + len(deutsch_verbs) + len(deutsch_adverbs))# + len(english_adjectives) + len(english_nouns) + len(english_verbs))  
#print(bool(len(deutsch_adjectives) == len(english_adjectives)))
print('english :',len(english_adjectives) , 'deutsch :' ,len(deutsch_adjectives))
print (translate_one('schmutzig'))
print (english_adjectives[39])
